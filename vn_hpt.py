...
data = {
        "Run2":ROOT.TFile("published/vnPtintegrated+Run2.root","read"),
		"Run3":ROOT.TFile("data/vn_run3_pass4_pt12-5.root","read"),
};
...
for i in range(0,1):
	for s in list(data)[::-1]:
		...
		if (s == "Run2"):
			gr_vn[(s,i)] = data[s].Get(("gr_v2intForPtbins05"))
		else:
...
ax1.text(0.05,0.32,"Pb-Pb $\\sqrt{s_\\mathrm{NN}}=5.02\\,\\mathrm{TeV}$\n$0.4<|\\eta|<0.8$, $1.2<p_\\mathrm{T}<5.0\\,\\mathrm{GeV/c}$",horizontalalignment="left",verticalalignment="center",transform=ax1.transAxes,size=9);

plot.Plot();
plot.Save("figs/vn_hpt.pdf");
plot.Show();
